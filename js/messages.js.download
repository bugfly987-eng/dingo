...
(function( require, define, factory ){
    ...
    var _get_messages = function() {
      ...
      var _escaped_url = encodeURIComponent('https://web.archive.org/web/20251021154110/http://pbskids.org' + _path);
      if (_url.search) _escaped_url += encodeURIComponent(_url.search);
      if (_url.hash) _escaped_url += encodeURIComponent(_url.hash);

      $.ajax({
        'url': '/api/messages/v3/messages.json?url=' + _escaped_url + '&n=' + _number_of_messages,
        'dataType': 'json',
        'complete': function(r) {
          if( r.status != 200 ){
    ...
    var _get_video_messages = function() {
      ...
      var videoCallback = function(e) {
        ...
        $.ajax({
          'url': '/api/messages/v3/messages.json?n=' + _DEFAULT_MESSAGES + '&nola=' + encodeURIComponent(e.release.series_nola) + '&v=' + _cosi_version[e.release.series_nola],
          'dataType': 'json',
          'complete': function(r) {
            if (r.status == 200) {
              ...
              if (video_content.title === "PBS KIDS") {
                console.warn('It looks like no NOLA Root is set for this show.');
              } else {
                var card = {
                  ...
                  'goals': video_content.curriculum.join(', '),
                  'subtype': tip.subtype,
                ...
                card.series = {
                  ...
                  'curriculum': video_content.curriculum,
...
/*
     FILE ARCHIVED ON 15:41:10 Oct 21, 2025 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 00:13:11 Feb 03, 2026.
...
playback timings (ms):
  ...
  esindex: 0.01
  cdx.remote: 13.633
  LoadShardBlock: 316.537 (6)
  PetaboxLoader3.datanode: 392.684 (8)
  load_resource: 421.883 (2)
  PetaboxLoader3.resolve: 163.554 (2)
*/