...
function getCmsRoot() {
  ...
  var cmsRoot = "https://web.archive.org/web/20251021154112/https://cms-tc.pbskids.org";
  ...
  if (href.match(/^https:\/\/.+\.preprod\.pbskids\.org/) || href.match(/^https:\/\/stage\.pbskids\.org/)) {
    cmsRoot = "https://web.archive.org/web/20251021154112/https://pbs-kids-cms-craft3-static-website-staging-tc.preprod.pbskids.org";
  }
...
(function( require, define, factory ){
  ...
  var osName = '';
  var imageBaseUrl = window._sponsor_img_root || getCmsRoot() + '/sponsorship/resources/img/';
  ...
  var removeBridgePageLinkClick = function (element) {
    // Remove all listeners on the element.
    console.log('Sponsors:: removeBridgePageLinkClick', element);
    jQuery(element).off('click', onBridgePageLinkClicked);
  }

  var validateForBridge = function (element, e){
    console.log('Sponsors:: validateForBridge', {
      element: element,
      e:e
    });

    if (
      bridgeURLs(element.hostname, element.pathname) === true &&
      bridgeURLs( window.location.hostname, window.location.pathname ) === false &&
      jQuery(element).attr('href').indexOf('javascript:') == -1
    ) {
      var pathnameslash = '';
      if( element.pathname.substring(0, 1) != '/' ) { pathnameslash = '/'; }

      var bridgeNotBuilt = bridge(
        jQuery(element).attr('href'),
        jQuery(element).attr('title'),
        jQuery(element).attr('class'),
        jQuery(element).attr('rel'),
        jQuery(element).attr('rev')
      );
  ...
  var addGenericBridgeModal = function (imageData) {
    jQuery('body').append([
      '<div id="bridgeContainer"></div><div id="bridgeOutline"></div>',
      '<div id="bridge"><div id="bridgeInner">',
        '<h3>You are now leaving PBS KIDS</h3>',
        '<p><a href="' + imageData.linkHref + '" title="' + imageData.linkHref + '" class="bridgeLink">Continue to ' + imageData.linkTitle + '&nbsp;&raquo;</a></p>',
        '<p><a href="' + imageData.linkHref + '" title="' + imageData.linkHref + '" class="bridgeSignLink">',
        '<img src="' + imageBaseUrl + 'bridge/sign.gif" id="sign" width="213" height="92" alt="" />',
        '<img src="' + imageData.src + '" id="' + imageData.id + '" style="' + imageData.style + '" alt="' + imageData.alt + '" />',
        '</a></p>',
        '<a href="" id="close" title="Back to PBS KIDS"><img src="' + imageBaseUrl + 'bridge/back-arrow.gif" width="55" height="67" alt="Back" /></a>',
      '</div></div>'
    ].join(''));
  ...
  var bridge = function (linkhref, linktitle, linkclass, linkrel, linkrev) {
    ...
    if ( !bridgeActive ) {
      ...
      bridgeTitleBackground = '#b3ce34 url(' + imageBaseUrl + 'bridge/youareleaving.gif) no-repeat 50% 50%';
      ...
      if ( bridgeTemplate == 'parents' || bridgeTemplate == 'parentsSection' ) {
        ...
        addGenericBridgeModal({
          id: 'parentsLogo',
          alt: 'PBS Parents Logo.',
          src: window._global_resources_root + 'img/pbs-kids-for-parents-logo.svg',
          style: 'width: 160px; height 38px; position: absolute; left: 20px; top: 14px',
          linkHref: bridgeLink,
          linkTitle: bridgeLinkTitle,
        });

      }
      else if ( bridgeTemplate == 'pbsorg') {

        addGenericBridgeModal({
          id: 'pbsLogo',
          alt: 'PBS Logo.',
          src: imageBaseUrl + 'bridge/PBS_Logo_160x38.png',
          style: 'width: 85px; height 38px; position: absolute; left: 57px; top: 14px',
          linkHref: bridgeLink,
          linkTitle: bridgeLinkTitle,
        });
        ...
        addGenericBridgeModal({
          id: 'learningMediaLogo',
          alt: 'PBS LearningMedia.',
          src:  window._global_resources_root + 'img/pbs-learning-media-blue.png',
          style: 'width: 100px; height: 33px; position: absolute; left: 50px; top: 14px;',
          linkHref: bridgeLink,
          linkTitle: bridgeLinkTitle,
        });
  ...
  var closeBridge = function(e) {
    ...
    jQuery('#bridge').animate({'opacity' : '0'}, 'fast', function() {
      jQuery('#bridgeContainer').animate({'opacity' : '0'}, 'fast', function() {
        ...
        jQuery('#overlayImageLink').remove();
        jQuery('#overlayHeadline').remove();
        jQuery('#overlayButton').remove();
        jQuery('#overlayText').remove();
  ...
  jQuery(document).ready(function(){
    ...
    jQuery('a, area').not('.not-bridge, .sponsor-validated').click(onBridgePageLinkClicked);
  ...
  function onBridgePageLinkClicked(e) {
    console.log('Sponsors:: onBridgePageLinkClicked');
  ...
  exports.onBridgePageLinkClicked = onBridgePageLinkClicked;
  exports.removeBridgePageLinkClick = removeBridgePageLinkClick;
...
playback timings (ms):
  ...
  esindex: 0.01
  cdx.remote: 54.593
  LoadShardBlock: 157.31 (3)
  PetaboxLoader3.datanode: 174.436 (4)
  load_resource: 97.113
