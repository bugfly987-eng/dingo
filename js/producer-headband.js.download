...
(function( require, define, factory ){
  ...
  if( typeof define === "function" && define.amd ){
    ...
    define( "headband", ["jquery", "require", "jquery-touch", "localization", "bridge-overlay"], factory );
  }
  else{
  ...
  *****************************/
  function onLocalized(e){
    if( e.station ){
      var _stationMarkup = '<a class="callsign" href=""><span class="alt-call-letters"></span><img src="" alt=" "/></a>';
      $("#headband-container .headband-nav .nav-item.station").html(_stationMarkup);
      $("#headband-container .headband-nav .nav-item.station a.callsign").attr({"href": (e.station.kidsSite || "" ) });
      $("#headband-container .headband-nav .nav-item.station .alt-call-letters").html(e.station.callLetters);
      $("#headband-container .headband-nav .nav-item.station img").attr({
        "src": (e.station['color-single-brand-logo'] || e.station['color-cobranded-logo'] || ""),
        "alt": (e.station.callLetters + " station logo.")
      });

      updateTVTimesLink( e.station );
    }
      ...
      if( type == "pbskids.messages.site"){
        ...
        if( !_hideExternalLinks ){
          ...
          for( var i = 0; i < messages.length ; i++ ){
            ...
            var __messageType = (__message.subtype || "daily-tip").replace(/\s+/g,"-");
            ...
            var content ={
              moreInfoType: __message.moreInfoType,
              content: __message.content,
              attributes: __message.attributes
            };

            parentsbar.controller.addItem( __parentsMessage, content, __message.type, __message.action_text || "", __message.link || "", __message.link_title || "", __message.tooltip_title || "" );
          }//end for var i
  ...
  *****************************/
  ...
  function updateTVTimesLink( station ){
    //Build TV Times link from localized station data
    var tvTimesUrl = station && (station.tvss_url || station.kidsSite || station.website_url);
    if(tvTimesUrl) {
      $("#headband-container .nav-item.tv-times").css("display", "").find("a").attr("href", tvTimesUrl);
    }
  }//updateTVTimesLink()
      ...
      $("<h3 class='series-title'></h3>" +
        ...
        "<div class='footer'>" +
          "<div class='related-links'>" +
            ...
            "<ul>" +
              "<li><a class='pbsparents'>PBS Parents</a></li>" +
              "<li><a class='pbsshop'>PBS Kids Shop</a></li>" +
              "<li><a class='pbsitunes'>PBS Kids on iTunes</a></li>" +
            "</ul>" +
          "</div>" +
        "</div>"
      ),
  ...
  function initController() {
     ...
     */
      ...
      **************************/
      ...
      //-- Modify Content ----
      var __buildLink = function (item, href, itemType, attributes, trackClickEvent) {
        var link = $(".more-toggle", item ).attr({ "href":href, "target": "_blank" });
        trackClickEvent = !(trackClickEvent === false);// default to `true` unless explicitly set to `false`.
        ...
        if (!trackClickEvent) {
          link.click(window.onBridgePageLinkClicked);
        }
        else {
          link.click(function(e){
            console.log('Sponsors:: __buildLink() > link.click()');

            if( item.hasClass("track") ){
              trackHeadbandEvent(
                itemType.replace("type-","") + " | " + $(".copy", item).text().toLowerCase(), //{Group-type}|{Message-type}
                href, //href url
                ( item.prop("data-index") + 1 ) //position in rotation
              );
            }
        ...
        console.log('Sponsors:: __buildLink() completed', link[0]);

        return link[0];
      };//__buildLink()

      var __buildToolTip = function (item, content, className, tooltipLink, tooltipLinkTitle, tooltipTitle) {
        var __onShow = function(){
          ...
          if( item.hasClass("track") ){
            trackHeadbandEvent(
              className.replace("type-","") + " | " + $(".copy", item).text().toLowerCase(), //{Group-type}|{Message-type}
              "Open Tooltip",
              ( item.prop("data-index") + 1 ) //position in rotation
            );
          }
        ...
        var __tip = new tooltips.Tooltip(__id, content, $(".more-toggle", item ), $(".parents-bar-tooltips"), __onShow, _enable, tooltipLink, tooltipLinkTitle, tooltipTitle);
        if( __tip && __tip.tip && className && typeof className == "string"){
            __tip.tip.addClass( className );
      ...
      var _addItem = function( item_arg, tooltip_arg, tooltipClass_arg, more_info_label, tooltipLink, tooltipLinkTitle, tooltipTitle){
        ...
        if( tooltip_arg ){
          ...
          if( typeof tooltip_arg != "string" && typeof tooltip_arg.moreInfoType == "string") {
            if (tooltip_arg.moreInfoType == "sponsorLink") {
              // Passing an actual url instead of '#' for the link href
              // results in a generic bridge page instead of the sponsor bridge page.
              var link = __buildLink(__item, tooltip_arg.content, tooltipClass_arg, tooltip_arg.attributes, false);

              if (typeof window.buildSponsorLinks === "function") {
                window.buildSponsorLinks(link);
              }
            }
            else if  (tooltip_arg.moreInfoType == "link") {
              __buildLink(__item, tooltip_arg.content, tooltipClass_arg, tooltip_arg.attributes);
            }
            else {
              __buildToolTip(__item, tooltip_arg.content, tooltipClass_arg, tooltipLink, tooltipLinkTitle, tooltipTitle);
            }
          }

          else {
            // Logic to handle legacy tooltip_arg:String.
            var urlRegex = /^https?:\/\//i;

            if( typeof tooltip_arg == "string" && urlRegex.test(tooltip_arg) ){
              __buildLink(__item, tooltip_arg, tooltipClass_arg);
            }

            else{
              __buildToolTip(__item, tooltip_arg, tooltipClass_arg, tooltipLink, tooltipLinkTitle, tooltipTitle);
            }
          }
        }
  ...
  function initSettings(){
     ...
     */
      ...
      **************************/
      ...
      var _addLocalizationModule = function(){
        if( org.pbskids.localization ){
          ...
          if( org.pbskids.localization.station ){
            ...
            __graphic     = org.pbskids.localization.station['color-single-brand-logo'] || org.pbskids.localization.station['color-cobranded-logo']  || "(none)";
            __linkout     = org.pbskids.localization.station.kidsSite || "";
              __graphicAlt  = (org.pbskids.localization.station.callLetters || "") + " station logo.";
          }
          else{
          ...
          var __module = _addModule( "Your Local PBS Station", __description, __buttonText, __settings, __graphic, __linkout, __graphicAlt);
          __module.module.addClass( "localization-module" );

          org.pbskids.localization.attachStationClickHandlers();
          ...
          var __onLocalized = function(e){
            if( e.station ){
              ...
              $(".logo img" , __module.module).attr({"src": e.station['color-single-brand-logo'] || e.station['color-cobranded-logo'], "alt": (e.station.callLetters + " station logo.")});
      ...
      var Module = function( title, description, buttonText, settingsContent, image, imagelinkoutURL, imageAltText ){
        var __module;
        ...
        var _init = function (){
          __module = $(
                        "<div class='" + _moduleClass + "'>"  +
                          ...
                          (image ? (imagelinkoutURL ? "<a href='" + imagelinkoutURL + "' " : "<span" ) + " class='logo'><img src='" + (image != "(none)" ? image : "") + "' alt='" + (imageAltText != "(none)" ? imageAltText : "") + "' />" + (imagelinkoutURL ? "</a>" : "</span>" ) : "") +
      ...
      var _addModule = function(title, description, buttonText, settingsContent, image, imagelinkoutURL, imageAltText){
        var __module = new Module(title, description, buttonText, settingsContent, image, imagelinkoutURL, imageAltText);
  ...
  {
    if(!url){
      $(this).attr({'href': "",'title': "" }).addClass("hidden");
    }
    else{
      $(this).attr({
        'href': url,
...
/*
     ...
     INTERNET ARCHIVE ON 00:13:12 Feb 03, 2026.
...
playback timings (ms):
  ...
  esindex: 0.011
  cdx.remote: 5.375
  LoadShardBlock: 312.904 (6)
  PetaboxLoader3.datanode: 318.8 (8)
  load_resource: 184.247 (2)
  PetaboxLoader3.resolve: 108.527 (2)
*/